...
;
; Haley Turner, Zachary Becker, EELE 465, Project 03
...
; R13- Register for CheckLockState Flag
; R14- Register for CheckLockState Counter
;
;-------------------------------------------------------------------------------
...
;-------------------------------------------------------------------------------
RESET       mov.w   #__STACK_END,SP         ; Init stackpointer
...
init:
	...
	bis.b	#BIT6, &P6DIR					; Init LED2 as output
	bic.b	#BIT6, &P6OUT					; Start LED2 off

; LED1 and LED2 on MSP430 used for temporary testing

	bis.b	#BIT0, &P6DIR					; Init P6.0 as output (LED 0)
	bic.b	#BIT0, &P6OUT					; Start P6.0 off

	bis.b	#BIT1, &P6DIR					; Init P6.1 as output (LED 1)
	bic.b	#BIT1, &P6OUT					; Start P6.1 off

	bis.b	#BIT2, &P6DIR					; Init P6.2 as output (LED 2)
	bic.b	#BIT2, &P6OUT					; Start P6.2 off

	bis.b	#BIT3, &P6DIR					; Init P6.3 as output (LED 3)
	bic.b	#BIT3, &P6OUT					; Start P6.3 off

	bis.b	#BIT4, &P6DIR					; Init P6.4 as output (LED 4)
	bic.b	#BIT4, &P6OUT					; Start P6.4 off

	bis.b	#BIT0, &P2DIR					; Init P2.0 as output (LED 5)
	bic.b	#BIT0, &P2OUT					; Start P2.0 off

	bis.b	#BIT1, &P2DIR					; Init P2.1 as output (LED 6)
	bic.b	#BIT1, &P2OUT					; Start P2.1 off

	bis.b	#BIT2, &P2DIR					; Init P2.2 as output (LED 7)
	...
	mov.w	#1, R13							; Init locked state
	mov.b	#4, R14							; set lock timer count (seconds) to zero
	...
	; Setup Timer Compare
	mov.w #32900, &TB0CCR0					; set compare value (SMCLK spec. @ 1 MHz)
...
;-------------------------------------------------------------------------------
main:
	mov.w	#0777h, R8					; Init outer delay loop counter (small delay for button response time)
	call	#Delay

	call 	#ColumnInput				; Change columns to be inputs, rows to be outputs
	call	#CheckKeypad				; Check if/what key is pressed

	cmp.b 	#1, R13						; Check if R13 is '1'
	jnz 	FindButton
;-------------------------- END main -------------------------------------------

;-------------------------------------------------------------------------------
; Subroutine: CheckLockState0
;-------------------------------------------------------------------------------
CheckLockState0:
	cmp.b 	#0, R4
	jnz		LockKeyPressed
...
;-------------------------------------------------------------------------------
; Subroutine: CheckLockState3
;-------------------------------------------------------------------------------
CheckLockState3:
	call	#SetLED2

	mov.w	#1777h, R8					; Init outer delay loop counter (small delay for button response time)
	call	#Delay
...
;-------------------------------------------------------------------------------
; Subroutine: LockKeyPressed
;-------------------------------------------------------------------------------
LockKeyPressed:
	mov.b	#4, R14
	bis.w #CCIE, &TB0CCTL0					; local enable for CCRO in TB0
	jmp		CheckLockState0_mid
;-------------------------- END LockKeyPressed ---------------------------------


;-------------------------------------------------------------------------------
; Subroutine: ResetLockSequence
;-------------------------------------------------------------------------------
ResetLockSequence:
	mov.b	#4, R14 						; reset Lock count to 2 seconds
	call	#ResetLED
	bic.w #CCIE, &TB0CCTL0					; local enable for CCRO in TB0
	jmp		main
;-------------------------- END ResetLockSequence ------------------------------


;-------------------------------------------------------------------------------
; Subroutine: CheckLockCount
;-------------------------------------------------------------------------------
CheckLockCount:
	cmp.b	#0, R14						; start over if 2 seconds has expired
	jz		ResetLockSequence
	ret
;-------------------------- END CheckLockCount ---------------------------------
...
;-------------------------------------------------------------------------------
RowInput:
	bic.b 	#BIT4, &P3DIR					; Init P3.4 as input
...
;-------------------------------------------------------------------------------
ColumnInput:
	bic.b 	#BIT0, &P3DIR					; Init P3.0 as input
...
;-------------------------------------------------------------------------------
ISR_TB0_CCR0:
	xor.b 	#BIT6, &P6OUT						; toggle P6.6 (Green LED)
	dec 	R14
	bic.w 	#CCIFG, &TB0CCTL0					; clear CCIFG flag
	reti
;-- End delay ------------------------------------------------------------------
